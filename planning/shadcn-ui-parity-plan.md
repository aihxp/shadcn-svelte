...
- Agent-facing commands: `search`, `view`, `info`, `docs`, `mcp`, `apply`, `preset`, `registry add`, and the deprecated `registry mcp` alias.
...
7. Phase 7, long tail: `apply`, `preset`, and `registry add` are complete; remaining work is `eject`, `migrate` (including the RTL migration from issue 2512), `diff` or unhidden `update`, top-level `build` alias, optional e2e fixture package.
